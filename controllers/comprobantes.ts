...
import { getTodayDateFormat } from "../helpers/date-values";
...
export const comprobanteNuevo = async (req: Request, res: Response) => {
    ...
    if(errorAbastecimiento){
        ...
        const billing : IComprobanteMaster = {
            ...
            tipo_comprobante,
            fecha_emision: getTodayDateFormat(fecha_emision),
            total_gravadas,
            total_igv,
            total_venta,
            pago_tarjeta,
...
export const getFechaUltimoComprobante = async (req: Request, res: Response) => {

    const { tipo_comprobante, proposito } = req.query;

    const { serie, hasError:hasErrorSerie, message:messageSerie } = await obtieneSerie(tipo_comprobante?tipo_comprobante.toString():'', proposito?proposito.toString():'')

    if(hasErrorSerie){ res.json({ hasError: true, respuesta: messageSerie}); return; }    

    try {
        const data: any = await obtieneFechaUltimoComprobante(serie);
...
export const obtieneDescuentos = async (req: Request, res: Response) => {
    ...
    const { hasError, message, descuento } = await getDescuentoPorItem( numero_documento, tipo, codigo_producto, items );
